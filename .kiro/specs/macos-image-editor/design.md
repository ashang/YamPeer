...
`FontBootstrapper` runs before the desktop host permits the normal `Primary_Main_Window` workspace to draw. It resolves the package-relative `resources/fonts/NotoSansCJKsc-Regular.otf` path through the platform package adapter, reads the complete resource, verifies that the byte stream contains the required font face, and constructs a `FontConfiguration` value. During eframe application construction, the host applies that value to `egui::FontDefinitions` through `CreationContext.egui_ctx.set_fonts` before the first workspace frame: the bundled face is inserted at the front of both the proportional and monospace family lists and remains in the fallback lists. This makes Chinese UI labels, Chinese filenames, and Chinese notices take the bundled face without depending on an installed system font, while preserving the normal `egui` fallback chain for non-required code points.
